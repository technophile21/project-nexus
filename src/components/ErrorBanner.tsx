...
export default function ErrorBanner({ warnings, onDismiss }: ErrorBannerProps) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-50 ${colors.bg} backdrop-blur-sm border-b ${colors.border} px-4 py-2.5`}
    >
      <div className="flex items-start gap-3 max-w-full">
        ...
        <div className="flex-1 min-w-0">
          {visible.map((w, i) => (
            <p key={i} className={`text-xs leading-5 ${colors.text}`}>
              {w.message}
            </p>
          ))}
          {overflow > 0 && (
            <p className={`text-xs leading-5 ${colors.muted}`}>
              +{overflow} more {overflow === 1 ? 'warning' : 'warnings'}
            </p>
          )}
        </div>
        ...
        <button
          onClick={onDismiss}
          className={`${colors.btn} shrink-0 rounded p-0.5 transition-colors`}
          aria-label="Dismiss"
        >
